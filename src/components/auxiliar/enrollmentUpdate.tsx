        ...
        <div className="form-control w-full">
            ...
            <div className="relative">
                <input
                    ref={inputRef}
                    type="text"
                    className={`input input-bordered w-full focus:input-primary transition-all ${disabled ? "bg-gray-100 text-gray-500 cursor-not-allowed" : ""
                        }`}
                    placeholder={label}
                    value={inputValue}
                    onChange={(e) => {
                ...
                {open && filteredOptions.length > 0 && !disabled && (
                    ...
                    >
                        {filteredOptions.map((o: string, index: number) => (
                            <div
                                key={`${o}-${index}`}
                                className={`px-4 py-2 cursor-pointer hover:bg-blue-500 hover:text-white ${o === value ? "bg-blue-500 text-white" : ""
                                    }`}
                                onMouseDown={(e) => {
                                    e.preventDefault();
                                    setValue(o);
...
export default function EnrollmentUpdate({
    ...
    return (
        <form className="space-y-4 bg-white" onSubmit={handleSubmit}>
            {successMsg && (
                <div
                    className={`font-semibold mb-2 p-2 rounded ${successMsg.includes("Error") || successMsg.includes("no tiene") ? "bg-red-50 text-red-600" : "bg-green-50 text-green-600"
                        }`}
                >
                    {successMsg}
                </div>
                ...
                <ComboBox
                    value={
                        selectedStudentEmail
                            ? studentOptions.find((opt: string) => opt.startsWith(selectedStudentEmail)) || ""
                            : ""
                    }
                    ...
                    required={true}
                />
                {students.length === 0 && <p className="text-xs text-warning">No hay estudiantes cargados.</p>}
            </div>
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    ...
                    <div className="flex justify-end gap-3 pt-4">
                        ...
                        <button
                            ...
                            disabled={loadingSubmit}
                        >
                            {loadingSubmit ? <span className="loading loading-spinner"></span> : "Actualizar Matrícula"}
                        </button>
                    </div>
                </div>
            )}
